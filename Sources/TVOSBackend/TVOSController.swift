...
/// Shared Appium-session operation for Simulator and physical Apple TV.
/// Session creation stays in the owning controller because physical devices
/// require preflight, signing-cache, and WDA lifecycle handling first.
enum TVOSTextInput {
    private static let textEntryRoles: Set<String> = [
        "TextField", "SecureTextField", "SearchField", "TextView",
    ]

    static func perform(_ text: String, in session: AppiumSession) async throws -> TVOSTypeResult {
        ...
        if !keyboardIsOpen {
            ...
            guard let focused, textEntryRoles.contains(focused.role) else {
