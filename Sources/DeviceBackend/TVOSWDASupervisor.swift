    ...
    private func probe(_ baseURL: URL) async -> Probe {
        ...
        do {
            ...
            guard let http = response as? HTTPURLResponse, (200..<300).contains(http.statusCode),
                  let value = try? JSONDecoder().decode(StatusEnvelope.self, from: data).value
            else {
                return .occupied(productBundleIdentifier: nil)
            }
            let product = value.build?.productBundleIdentifier
            if value.ready == true || value.state == "success" {
