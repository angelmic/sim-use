...
| iOS device: installed WDA signature expired or runner is missing | The signed XCTest runner is no longer launchable | Set `SIM_USE_WDA_BUNDLE_ID` to the installed product id and `SIM_USE_XCODE_ORG_ID` to its Apple Developer Team. sim-use then checks the per-UDID signing timestamp/fingerprint before session creation: a valid artifact uses `test-without-building`, while a missing/stale one performs one incremental sign/build repair without first waiting for the preinstalled timeout. Inspect `~/.sim-use/<UDID>/wda-signing-cache.json` |
