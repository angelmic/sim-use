...
Frames in the JSON output (`--json`: `entries[].frame`, `screen`) are in platform-native units — iOS **points**, tvOS WebDriver display coordinates, Android **pixels**. Key off the envelope's `platform` field before doing math on coordinates across platforms.
...
| tvOS: focus does not move | Direction is unavailable from the current focus graph, or an animation is still settling | Re-run `ui`, choose another direction, and verify the `focused` entry |
